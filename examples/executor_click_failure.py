...
TRACE_ROOT = PROJECT_ROOT / "trace" / "executor-click-failure"
TARGET_SELECTOR = "#missing-target"
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        with RecordedPlaywrightExecutor(page, trace_dir) as executor:
            action = executor.click(TARGET_SELECTOR, timeout_ms=500)

        browser.close()

    if (
        action.status is not ActionStatus.FAILURE
        or action.failure_category is not FailureCategory.TARGET_NOT_FOUND
    ):
        raise RuntimeError("the missing-target diagnosis was not recorded")
